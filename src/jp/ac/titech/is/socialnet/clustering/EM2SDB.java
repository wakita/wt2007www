...
public class EM2SDB extends SDBBuilder {
  enum Parameters {
    EM, SDB
  }
  ...
  private void usage() {
    ...
    error_message
        .append("java jp.ac.titech.is.socialnet.clustering.EM2SDB --em <path> --sdb <path>\n");
    error_message.append("-----\n");
    error_message
        .append("EM2SDB converts an edge list file specified by the \"--em\" paramter to a smard DB file that comprizes of an index file (*.idx) and a db file (*.db).  The path to the smart DB file is specified by removing the filename extension from the pathname to *.idx or *.db file.");
  ...
  private void run(String[] args) {
    ...
    try {
      File csv_path = new File(params.get(Parameters.EM));
