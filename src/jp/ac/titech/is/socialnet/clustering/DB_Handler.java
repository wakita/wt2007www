...
public class DB_Handler {
  ...
  public DB_Handler(String idx, String db, int max_id) {
    Index_name = idx;
    DB_name = db;
    MAX_ID = max_id;
  }

  public DB_Handler(File prefix, int size) {
    this(prefix.toString() + ".idx", prefix.toString() + ".db", size);
  }

  public DB_Handler(File prefix) {
    this(prefix.toString() + ".idx", prefix.toString() + ".db", 0);
  }

  public void make_dQ_DB(long[][] neighborsDQ) throws IOException {
    if (Main.debug) System.out.println("Creating the dqDB and dqIndex");
    ...
    int idx_size;
    idx_size = dq_arr.length * INTEGER_SIZE + SURPLUS_SIZE;
    int db_size = dq_info_num * LONG_SIZE;
    ...
    MappedByteBuffer idx = idx_chan.map(FileChannel.MapMode.READ_WRITE, 0,
                                        idx_size);
    MappedByteBuffer db = db_chan.map(FileChannel.MapMode.READ_WRITE, 0,
                                      db_size);
    ...
    idx_chan.close();
    db_chan.close();
    if (Main.debug) System.out.println("Created the dqDB and dqIndex");
  }

  public void make_DB(int[][] sorted_link) throws IOException {
    if (Main.debug) System.out.println("Creating the DB and Index");
    ...
    MappedByteBuffer idx = idx_chan.map(FileChannel.MapMode.READ_WRITE, 0,
                                        IDXSIZE);
    MappedByteBuffer db = db_chan
        .map(FileChannel.MapMode.READ_WRITE, 0, DBSIZE);
    ...
    idx_chan.close();
    db_chan.close();
    if (Main.debug) System.out.println("Created the DB and Index");
  }
  ...
  public int[] neighbors(int i) throws IOException {
    ...
    MappedByteBuffer idx = idx_chan.map(FileChannel.MapMode.READ_ONLY, 0,
                                        IDXSIZE);
    MappedByteBuffer db = db_chan.map(FileChannel.MapMode.READ_ONLY, 0, DBSIZE);
    long n_entries = idx.getInt(0);
    if (i < 0 || n_entries < i) {
      if (Main.debug) System.out.println("out of range!!");
  ...
  public int[][] load() throws IOException {
    FileChannel idx_chan = new RandomAccessFile(Index_name, "r").getChannel();
    FileChannel db_chan = new RandomAccessFile(DB_name, "r").getChannel();
    // System.out.printf("estimated max_id = %d\n", idx_chan.size() /
    // Constants.ISIZE - 2);
    this.MAX_ID = (int) (idx_chan.size() / Constants.ISIZE - 2);
    long IDXSIZE = new File(Index_name).length();
    long DBSIZE = new File(DB_name).length();
    MappedByteBuffer idx = idx_chan.map(FileChannel.MapMode.READ_ONLY, 0,
                                        IDXSIZE);
    ...
    long n_entries = idx.getInt(0);
    if (Main.debug) System.out.println("#entries = " + n_entries);
